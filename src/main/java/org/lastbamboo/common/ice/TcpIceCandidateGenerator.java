 ...
 */
public class TcpIceCandidateGenerator implements IceCandidateGenerator
    {
     ...
     */
    public TcpIceCandidateGenerator(final BindingTracker tracker, 
    ...
    public byte[] generateCandidates() throws IOException
        ...
        try
            {
            final SessionDescription sdp = createSdp();
            return sdp.toBytes();
            }
        catch (final SdpException e)
            {
            final IOException ioe = new IOException("Could not create SDP");
            ioe.initCause(e);
            throw ioe;
            }
        }
    
    private SessionDescription createSdp() throws SdpException
        {
